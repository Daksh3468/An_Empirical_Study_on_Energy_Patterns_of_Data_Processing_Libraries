...
csv_handler = CSVHandler(f"polars_adult_v0.20.0_itr(30).csv")
...
def replaces(df):
    return df.with_columns(
        pl.col("workclass").replace({"?": "X", "Private": "P"}).alias("workclass")
    )
...
print(f"Starting adult Polars Process...")

for i in range(30):
    df = load_csv(f'../datasets/adult.csv')
    sleep()
    df = load_json(f'../datasets/adult.json')
    sleep()

    save_csv(df, f'df_adult_polars_{i}.csv')
    sleep()
    save_json(df, f'df_adult_polars_{i}.json')
    sleep()

    clear_caches()
    df = pl.read_csv(f'../datasets/adult.csv')
    sleep()
    isna(df, 'workclass')
    sleep()
    dropna(df)
    ...
    sleep()
    drop(df, ['age', 'education'])
    sleep()
    groupby(df, 'workclass')
    sleep()
